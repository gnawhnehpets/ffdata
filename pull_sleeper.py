...
def gameData (url, season, week, db):
    print("WWW")
    ...
    doc = {
            ...
            'metadata': {
                ...
                'home_first_downs': home_actual['first_downs'], 
                'home_total_yards': home_actual['total_yards'], 
                'home_turnovers': home_actual['turnovers'], 
                'home_penalties': home_actual['penalties'], 
                'home_penalty_yards': home_actual['penalty_yards'], 
                'home_sacks': home_actual['sacks'], 
                'home_time_of_possession': home_actual['time_of_possession'], 
                'away_first_downs': away_actual['first_downs'], 
                'away_total_yards': away_actual['total_yards'], 
                'away_turnovers': away_actual['turnovers'], 
                'away_penalties': away_actual['penalties'], 
                'away_penalty_yards': away_actual['penalty_yards'], 
                'away_sacks': away_actual['sacks'], 
                'away_time_of_possession': away_actual['time_of_possession']
            },
...
db = client['ffdata']
db.gamedata.drop()
db.create_collection('gamedata')

season = 2021 # doing 1 season at a time, you can obviously loop this, but remember 2021 has 1 extra week of games
...
for week in range(1, 22):
    ...
    for game in soup.select('div.game_summary'):
        ...
        tmp = link[0]['href']
        print(tmp)
        gameData(tmp, season, wk, db)
        # try:
        #     gameData(link[0]['href'], season, wk, db)
        # except:
        #     pass
        print(".", end="")
